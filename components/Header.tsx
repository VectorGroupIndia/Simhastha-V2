...
const Header: React.FC = () => {
  ...
  const navLinkClass = ({ isActive }: { isActive: boolean }) =>
    `text-sm font-semibold leading-6 transition-colors ${isActive ? 'text-brand-primary' : 'text-slate-700 hover:text-brand-primary'}`;
      ...
      <nav className="container mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between py-4" aria-label="Global">
        <div className="flex lg:flex-1">
          <Link to="/" className="-m-1.5 p-1.5">
            ...
            <span className="text-2xl font-bold text-brand-dark">found<span className="text-brand-primary">tastic</span></span>
          </Link>
        </div>
        ...
        <div className="hidden lg:flex lg:flex-1 lg:justify-end items-center gap-x-6">
          <div className="relative">
            <button onClick={() => setLangDropdownOpen(!isLangDropdownOpen)} className="flex items-center gap-x-1 text-sm font-semibold text-slate-700 hover:text-brand-primary">
            ...
            {isLangDropdownOpen && (
              <div className="absolute right-0 mt-2 w-28 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                <div className="py-1">
                  <button onClick={() => { setLanguage('English'); setLangDropdownOpen(false); }} className="text-slate-700 block w-full text-left px-4 py-2 text-sm hover:bg-slate-100">English</button>
                  <button onClick={() => { setLanguage('हिंदी'); setLangDropdownOpen(false); }} className="text-slate-700 block w-full text-left px-4 py-2 text-sm hover:bg-slate-100">हिंदी</button>
                  <button onClick={() => { setLanguage('मराठी'); setLangDropdownOpen(false); }} className="text-slate-700 block w-full text-left px-4 py-2 text-sm hover:bg-slate-100">मराठी</button>
                </div>
              </div>
            )}
          </div>
          {user ? (
            ...
            <NavLink to="/login" className="text-sm font-semibold leading-6 text-slate-700 hover:text-brand-primary">
              {t.navLogin} <span aria-hidden="true">&rarr;</span>
            </NavLink>
          )}
        </div>
      </nav>
          ...
          <div className="mt-6 flow-root">
            <div className="-my-6 divide-y divide-slate-500/10">
              <div className="space-y-2 py-6">
                <NavLink to="/" onClick={() => setIsMenuOpen(false)} className="-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-slate-900 hover:bg-slate-50">{t.navHome}</NavLink>
              ...
              <div className="py-6">
                {user ? (
                  <>
                   {dashboardLink && <NavLink to={dashboardLink.path} onClick={() => setIsMenuOpen(false)} className="-mx-3 block rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-slate-900 hover:bg-slate-50">{dashboardLink.name}</NavLink>}
                   <button onClick={() => { logout(); setIsMenuOpen(false); }} className="-mx-3 block w-full text-left rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-slate-900 hover:bg-slate-50">
                      {t.navLogout}
                   </button>
                  </>
                ) : (
